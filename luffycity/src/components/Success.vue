  ...
  <div class="success">
    ...
    <div class="main">
      ...
      <div class="order-info">
        ...
        <p class="info3"><b>课程信息：</b><span v-for="(i,j) in course_list" >
          <span>《{{i.name}}》</span></span></p>
      </div>
      ...
      this.$axios.get(`${this.$settings.Host}/payments/alipay/result/` + location.search)
        .then(res => {
          ...
          if (localStorage.user_credit){
            localStorage.user_credit = parseInt(res.data.credit)
          }
          else {
            sessionStorage.user_credit = parseInt(res.data.credit)
          }
        })
        .catch(error => {
